...
class DGOSFET_numerical_model_v1_0_0():
    """
    ...
    def __init__(self, params):
        ...
        """
        self.mu_band = params['mu_band']  # band mobility ( constant mobility assumed for extended states above the band edge ) in m^2.V^-1.s^-1
        ...
        self.kfun = lambda x: self.kDE if x <= self.tDE or x >= self.tDE + self.tCH else self.kCH  # dielectric constant as function of x
        ...
        self.poisson_mat[self.grid_size - 1, self.grid_size - 1] = 1
...
if __name__ == "__main__":
    ...
    for ii in range(len(Vds)):
        ...
        osfet_dut_0 = DGOSFET_numerical_model_v1_0_0(params=num_params)
    ...
    legend = [None]
    logy_lin_plot(x=Vgs_all_mod, y=Id_all_mod, c=['k', 'r'], lw=4, s=['None', 'solid'], m=['o', 'None'], xlabel="$V_{GS} (V)$", ylabel="$Id (\mu A/\mu m)$", l=legend, ylim=[1e-8, 1e1], figname='Id_DGFET.png')
    logy_lin_plot(x=Vgs_all_mod[1:2] * 2, y=Qch_all_mod[:1] + Qf_all_mod[:1], c=['b', 'b'], lw=4, s=['solid', 'dashed'], m=['None', 'None'], xlabel="$V_{GS} (V)$", ylabel="$Qch (C)$", l=legend, figname='Qch_DGFET.png')
